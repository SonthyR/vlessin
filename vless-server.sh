...
# Web 面板配置
readonly PANEL_PORT="${PANEL_PORT:-8080}"
readonly PANEL_HOST="${PANEL_HOST:-0.0.0.0}"
readonly PANEL_DIR="$CFG/panel"
readonly PANEL_PID_FILE="$CFG/panel.pid"
...
# 安全的临时文件创建
_safe_temp_file() {
    local prefix="${1:-vless}"
    local tmp
    if command -v mktemp &>/dev/null; then
        tmp=$(mktemp -t "${prefix}.XXXXXX" 2>/dev/null || mktemp)
    else
        tmp="/tmp/${prefix}.$$.$(date +%s)"
    fi
...
_db_apply() { # _db_apply [jq args...] 'filter'
    [[ -f "$DB_FILE" ]] || init_db || return 1
    local tmp; tmp=$(_safe_temp_file "db_apply") || return 1
    local cleanup=1
    trap "[[ \$cleanup -eq 1 ]] && rm -f '$tmp'" EXIT INT TERM
    
    if jq "$@" "$DB_FILE" >"$tmp" 2>/dev/null; then
        mv "$tmp" "$DB_FILE" || { cleanup=0; rm -f "$tmp"; return 1; }
        cleanup=0
        _db_touch
        _db_clear_cache  # 清除缓存，确保数据一致性
        trap - EXIT INT TERM
        return 0
    fi
    cleanup=0
...
# 批量数据库操作（优化性能）
# 用法: db_batch_update '{"xray":{"vless":{"port":443}}}'
db_batch_update() {
    local updates="$1"
    [[ ! -f "$DB_FILE" ]] && init_db || return 1
    ...
    if ! echo "$updates" | jq empty 2>/dev/null; then
        _err "db_batch_update: 无效的 JSON 格式"
        return 1
    fi
    
    local tmp; tmp=$(_safe_temp_file "db_batch") || return 1
    local cleanup=1
    trap "[[ \$cleanup -eq 1 ]] && rm -f '$tmp'" EXIT INT TERM
    
    # 深度合并更新
    if jq --argjson updates "$updates" '
        def deep_merge(a; b):
            a as $a | b as $b
            | if ($a | type) == "object" and ($b | type) == "object" then
                [$a, $b] | add | with_entries(.value = deep_merge($a[.key]; $b[.key]))
              else
                $b
              end;
        deep_merge(.; $updates)
...
# 查询 Xray Stats API（安全版本，消除 eval）
...
xray_api_query() {
    ...
    # 使用数组传递参数，避免 eval
    local cmd_args=("xray" "api" "statsquery" "--server=127.0.0.1:${XRAY_API_PORT}")
    [[ "$reset" == "true" ]] && cmd_args+=("-reset")
    [[ -n "$pattern" ]] && cmd_args+=("-pattern" "$pattern")
...
_save_join_info() {
    ...
    for label in V4 V6; do
        ...
        if [[ "$cmd" =~ ^([a-zA-Z_][a-zA-Z0-9_]*) ]]; then
            ...
            local func_args="${cmd#$func_name}"
            # 使用函数调用代替 eval
            if declare -f "$func_name" >/dev/null 2>&1; then
                link=$($func_name $func_args 2>/dev/null)
            else
                _warn "函数 $func_name 不存在，跳过链接生成"
                link=""
            fi
        else
            _warn "无法解析命令: $cmd"
...
cleanup_hy2_nat_rules() {
    ...
    for chain in PREROUTING OUTPUT; do
        iptables -t nat -S $chain 2>/dev/null | grep -E "REDIRECT.*--to-ports" | while read -r rule; do
            # 将 -A 替换为 -D，然后安全地执行
            local delete_rule=$(echo "$rule" | sed 's/^-A/-D/')
            # 解析规则并安全执行
            if [[ "$delete_rule" =~ ^-D[[:space:]]+([^[:space:]]+) ]]; then
                local chain_name="${BASH_REMATCH[1]}"
                local rest="${delete_rule#-D $chain_name}"
                iptables -t nat -D "$chain_name" $rest 2>/dev/null
            fi
        done
    done
}
...
_issue_cert_dns() {
    ...
    # 安全设置环境变量（避免 eval）
    # dns_env 格式: "KEY1=value1 KEY2=value2" 或 "KEY=value"
    while IFS='=' read -r key value; do
        [[ -n "$key" && -n "$value" ]] && export "$key=$value"
    done <<< "$(echo "$dns_env" | sed 's/ /\n/g')"
...
_display_sorted_latencies() {
    ...
    sort -t'|' -k1 -n "$results" | while IFS='|' read -r _ latency name type server port; do
        ...
        # 如果提供了标记数组名，尝试获取对应的标记（使用间接引用代替 eval）
        if [[ -n "$marks_array_name" ]]; then
            local array_ref="${marks_array_name}[${name}]"
            local mark_value="${!array_ref}"
    ...
    while true; do
        ...
        if [[ -n "$installed" ]]; then
            case $choice in
                ...
                12) do_update ;;
                13) do_uninstall ;;
                14) manage_panel; skip_pause=true ;;
                0) exit 0 ;;
...
start_panel() {
    if [[ -f "$PANEL_PID_FILE" ]]; then
        local pid=$(cat "$PANEL_PID_FILE" 2>/dev/null)
        if kill -0 "$pid" 2>/dev/null; then
            _warn "Web 面板已在运行 (PID: $pid)"
            return 0
        else
            rm -f "$PANEL_PID_FILE"
        fi
    ...
    if ! command -v python3 &>/dev/null; then
        _err "未找到 python3，请先安装 Python 3"
        return 1
    fi
    ...
    if ! python3 -c "import flask" 2>/dev/null; then
        ...
        pip3 install flask flask-cors --quiet 2>/dev/null || {
            _err "安装 Flask 失败，请手动安装: pip3 install flask flask-cors"
            return 1
        }
    fi
    ...
    local panel_source="$script_dir/panel"
    local panel_server="$script_dir/panel_server.py"
    
    if [[ -d "$panel_source" ]]; then
        mkdir -p "$PANEL_DIR"
        cp -r "$panel_source"/* "$PANEL_DIR/" 2>/dev/null
        _ok "面板文件已复制到 $PANEL_DIR"
    fi
    
    if [[ ! -f "$panel_server" ]]; then
        _err "未找到面板服务器文件: $panel_server"
        return 1
    fi
    
    # 启动面板服务器
    chmod +x "$panel_server" 2>/dev/null
    nohup python3 "$panel_server" > "$CFG/panel.log" 2>&1 &
    local pid=$!
    ...
    if kill -0 "$pid" 2>/dev/null; then
        _ok "Web 面板已启动"
        _info "访问地址: http://${PANEL_HOST}:${PANEL_PORT}"
        _info "PID: $pid"
        _info "日志文件: $CFG/panel.log"
        return 0
    else
        _err "Web 面板启动失败，请查看日志: $CFG/panel.log"
        rm -f "$PANEL_PID_FILE"
        return 1
    fi
}
...
manage_panel() {
    while true; do
        ...
        case $choice in
            1) start_panel; _pause ;;
            2) stop_panel; _pause ;;
            3) restart_panel; _pause ;;
            4) 
                if [[ -f "$CFG/panel.log" ]]; then
                    tail -n 50 "$CFG/panel.log"
                else
                    _warn "日志文件不存在"
                fi
                _pause
...
case "${1:-}" in
    ...
    --help|-h)
        ...
        echo "  --panel-start     启动 Web 管理面板"
        echo "  --panel-stop      停止 Web 管理面板"
        echo "  --panel-status    查看 Web 面板状态"
        echo "  --panel-restart   重启 Web 管理面板"
        echo "  --help, -h        显示帮助信息"
        echo ""
        echo "无参数时启动交互式菜单"
        exit 0
